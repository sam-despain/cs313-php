...
<html>
	<body>
		...
		foreach ($db->query($myQuery) as $row)
		{
			...
			echo '<button class="dropButton, fittedDiv">Edit</button>';
			...
			echo '<option></option>';
			echo '<?php';
			echo 'foreach ($db->query(\'SELECT name FROM type ORDER BY name;\') as $row){';
			echo 'echo \'<option>\' . $row[\'name\'] . \'</option>\';';
			echo '}';
			echo '?>';
			echo '</select>';
			echo '</p>';
			...
			echo '</form>';
			echo '<button class="dropButton, fittedDiv">Remove</button>';
